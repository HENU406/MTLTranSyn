...
class MTLTranSyn(Module):
    ...
    def forward(self, d1, d2, c_exp):
        ...
        tower_input1 = torch.sum(gates_o.t().unsqueeze(2).expand(-1, -1, self.config['final_output_size']) * experts_o_tensor,
                                 dim=0)
        tower_input2 = torch.sum(
            gates_o1.t().unsqueeze(2).expand(-1, -1, self.config['final_output_size']) * experts_o_tensor1, dim=0)
